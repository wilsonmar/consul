...
See the [Architecture](/docs/ecs/architecture) page for an overview of task startup, task shutdown, ACL tokens, and the
...
is an IAM role associated with an ECS task. The `consul login` command obtains credentials for this
role and uses those credentials to sign the login request to the AWS IAM auth method. This proves
the ECS task's identity to the Consul servers.
...
* <EnterpriseAlert inline /> A <code>consul.hashicorp.com.namespace</code> tag on the task role
  indicating the namespace where this service is registering.
