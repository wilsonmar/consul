...
its own instance of the ACL controller.
...
| Bootstrap ACL Token   | `my-consul-bootstrap-token`  | An ACL token with `acl:write` and `operator:write` permissions.                                                                                                       |
## Task Definition
...
| `secrets`     | list    | Must set `CONSUL_HTTP_TOKEN` to a Consul ACL token and, if applicable, set `CONSUL_CACERT_PEM` if a Consul server CA certificate is needed. |
| `environment` | string  | Must set `CONSUL_HTTP_ADDR` to the address of the HTTP API of your Consul servers.                                                          |

The following CLI options are available in the `command` field of the container definition.

| Flag                        | Type    | Description                                                                                                          |
| --------------------------- | ------- | -------------------------------------------------------------------------------------------------------------------- |
| `-iam-role-path`            | string  | IAM roles at this path will be trusted by the AWS IAM auth method created by the controller.                         |
